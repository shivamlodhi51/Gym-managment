...
{
    mysqli_query($conn, "UPDATE `register` SET fullname = '$fullname', email = '$email',
    no = $no, dob = '$dob', password = '$password' , cpassword = '$cpassword' WHERE id = $id") or die('query failed');
    echo "<script>alert('Profile updated successfully...'); window.location.href='profile.php';</script>";
}
    else{
        echo "<script>alert('Confrim password does not match'); window.location.href='profile.php';</script>";
    }
}
?>
<!DOCTYPE html>
                    ...
                    <div class="pbox w-100">
                        <form action="" class="w-100" method="post">
                            <!-- <div class="pimg w-100 d-flex justify-content-center">
                                <img class="" src="<?php echo $row['ppic']; ?>" width="200px" height="200px" alt="">
                            </div> -->
                            <!-- <div class="pname">
                                <h2 class=""><?php echo $row['pname']; ?></h2>
                            ...
                            <div class="buttn">
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control" name="fullname" required value="<?php echo $nrow['fullname']; ?>">
                                </div>
                                <div class="form-group">
                                    <label for="exampleInputEmail3">Email</label>
                                    <input type="text" class="form-control" required name="email" value="<?php echo $nrow['email']; ?>">
                                </div>
                               
                                <div class="form-group">
                                    <label for="city">Number</label>
                                    <input type="text" class="form-control" name="no" value="<?php echo $nrow['no']; ?>" min="1" max="5" required>
                                </div>
                                <div class="form-group">
                                    <label for="exampleInputEmail3">Gender</label>
                                    <select class="form-control w-100" id="inputState" value="<?php echo $nrow['gender']; ?>" required name="gender" id="">
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="date" class="form-control" name="dob" value="<?php echo $nrow['dob']; ?>" min="1" max="5" required>
                                </div>
                                <div class="form-group">
                                    <label for="city">Password</label>
                                    <input type="Password" class="form-control" name="password" value="<?php echo $nrow['password']; ?>" min="1" max="5" required>
                                </div>
                                <div class="form-group">
                                    <label for="city">Confirm Password</label>
                                    <input type="Password" class="form-control" name="cpassword" value="<?php echo $nrow['cpassword']; ?>" min="1" max="5" required>
                                </div>
                                <div class="form-group w-100">
                                    <button style="font-size: 13px;" type="submit" class=" w-100 btn btn-primary" name="update">UPDATE</button>
                                </div>
                        </form>
                    </div>
                </div>
